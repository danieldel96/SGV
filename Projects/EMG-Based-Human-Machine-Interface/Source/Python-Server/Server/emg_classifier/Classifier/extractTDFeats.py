...
def extractTDFeats(sample):
    ...
    for sig in sample:
        ...
        sig = (sig - np.ones((1,sig.size)) * np.mean(sig))[0]
        ...
        for i in range(1,frame_len-1):
            ...
            if((mid>=0 and fst>=0) or (mid<=0 and fst<=0)):
                flag1 = flag2
        ...
        #normalize for LDA
        length = length / frame_len
        turns = turns/frame_len
        zero_count = zero_count/frame_len
...
def main():   
    np_load = np.load('npz_files/raw_data.npz',allow_pickle=True) # load
    feature_array = []
    ...
    np.savez('npz_files/feat_data.npz', feature_array = feature_array, dataset_class = dataset_class)
